...
def train(args, train_dataset, eval_dataset, teacher_model, student_model, tokenizer):
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataloader):  # (input_ids, token_type_ids, attention_mask, img_feat, labels)
            ...
            teacher_cls_outputs = [
                teacher_reps[i * layers_per_block][:, 0] for i in range(student_layer_num + 1)
            ]
            student_cls_outputs = [student_reps[i][:, 0] for i in range(len(student_reps))]
            teacher_patience = torch.stack(teacher_cls_outputs[:-1]).transpose(0, 1)
            student_patience = torch.stack(student_cls_outputs[:-1]).transpose(0, 1)
            teacher_patience = F.normalize(teacher_patience, p=2, dim=-1)
            student_patience = F.normalize(student_patience, p=2, dim=-1)
            # L_pkd
            pkd_loss = F.mse_loss(teacher_patience, student_patience).half()
            ...
            # L = (1 - α) * L_ce + α * L_kd + β * L_pkd
            loss = (1 - args.alpha) * ce_loss + args.alpha * kd_loss + args.beta * pkd_loss  # L(x;θ_s;θ_t)
