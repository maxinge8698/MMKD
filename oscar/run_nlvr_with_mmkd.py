...
class NLVRDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for choice_key in entry['img_key']:  # ("left", "right")
            ...
            input_ids, segment_ids, input_mask, img_feat, input_mask_txt, input_mask_img = self.tensorize_example(text_a=entry['text_a'], img_feat=img_feat, text_b=entry['text_b'][choice_key])
            choices.append((input_ids, segment_ids, input_mask, img_feat, input_mask_txt, input_mask_img))
        # print(choices)
        '''
        [
            (
        ...
        choice_input_mask_txt = [choice[4] for choice in choices]  # [长度为95的list, 长度为95的list]
        choice_input_mask_img = [choice[5] for choice in choices]  # [长度为95的list, 长度为95的list]
        ...
        return (
            ...
            # #
            torch.tensor(choice_input_mask_txt, dtype=torch.long),
            torch.tensor(choice_input_mask_img, dtype=torch.long)
        )  # input_ids, token_type_ids, attention_mask, img_feat, labels, attention_mask_txt, attention_mask_img
...
def train(args, train_dataset, eval_dataset, teacher_model, student_model, tokenizer):
    ...
    for epoch in range(int(args.num_train_epochs)):
        t_start = time.time()
        for step, batch in enumerate(train_dataloader):  # (input_ids, token_type_ids, attention_mask, img_feat, labels, attention_mask_txt, attention_mask_img)
            # MMKD
            batch = tuple(t.to(args.device) for t in batch)  # (input_ids, token_type_ids, attention_mask, img_feat, labels, attention_mask_txt, attention_mask_img)
            inputs = {
                ...
                'labels': batch[4],
                #
                'attention_mask_txt': batch[5],
                'attention_mask_img': batch[6]
            }
...
def evaluate(args, model, eval_dataset=None, prefix=""):
    ...
    eval_steps = 0
    t_start = time.time()
    for batch in tqdm(eval_dataloader, desc="Evaluating"):  # (input_ids, token_type_ids, attention_mask, img_feat, labels, attention_mask_txt, attention_mask_img)
        batch = tuple(t.to(args.device) for t in batch)  # (input_ids, token_type_ids, attention_mask, img_feat, labels, attention_mask_txt, attention_mask_img)
        inputs = {
            'input_ids': batch[0],
